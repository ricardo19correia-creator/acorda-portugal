...
exports.CreatorsHero = CreatorsHero;
var react_1 = require("react");
var lucide_react_1 = require("lucide-react");
function CreatorsHero(_a) {
    var onOpenCreateModal = _a.onOpenCreateModal, onSelectHighlights = _a.onSelectHighlights, totalPostsCount = _a.totalPostsCount;
    return (<section className="relative overflow-hidden rounded-3xl border border-white/15 bg-gradient-to-b from-slate-900/90 via-slate-950/95 to-slate-950 p-6 sm:p-10 shadow-2xl backdrop-blur-xl">
      ...
      <div className="relative z-10 mx-auto max-w-4xl text-center">
        ...
        <div className="mt-6 sm:mt-8 flex flex-wrap items-center justify-center gap-3 sm:gap-4">
          <button type="button" onClick={onOpenCreateModal} className="group relative inline-flex items-center justify-center gap-2.5 rounded-2xl bg-gradient-to-r from-emerald-500 to-teal-500 px-6 py-3.5 text-sm font-black uppercase tracking-wider text-slate-950 shadow-[0_0_25px_rgba(16,185,129,0.4)] transition-all hover:scale-105 hover:from-emerald-400 hover:to-teal-400 active:scale-95 cursor-pointer">
            <lucide_react_1.PenSquare className="h-4 w-4 transition-transform group-hover:rotate-12"/>
            <span>✍️ Criar Publicação</span>
          </button>

          <button type="button" onClick={onSelectHighlights} className="inline-flex items-center justify-center gap-2 rounded-2xl border border-amber-500/40 bg-amber-500/10 px-6 py-3.5 text-sm font-bold text-amber-300 shadow-md backdrop-blur-md transition-all hover:bg-amber-500/20 hover:border-amber-400 active:scale-95 cursor-pointer">
            <lucide_react_1.Flame className="h-4 w-4 text-amber-400"/>
