...
exports.CreatorsSidebar = CreatorsSidebar;
...
function CreatorsSidebar(_a) {
    var onSelectTopic = _a.onSelectTopic, onOpenCreateForChallenge = _a.onOpenCreateForChallenge;
    return (<aside className="space-y-6">
      ...
      <div className="relative overflow-hidden rounded-3xl border border-amber-500/40 bg-gradient-to-b from-amber-950/40 via-slate-900 to-slate-950 p-5 shadow-xl backdrop-blur-md">
        ...
        <div className="flex items-center gap-2 text-xs font-black uppercase tracking-wider text-amber-300">
          ...
          <span>{creators_service_1.DAILY_CHALLENGE.title}</span>
        </div>

        <p className="mt-2.5 font-display text-sm font-black text-white leading-snug">
          "{creators_service_1.DAILY_CHALLENGE.question}"
        </p>

        {creators_service_1.DAILY_CHALLENGE.featuredResponse && (<div className="mt-3 rounded-2xl border border-white/10 bg-slate-950/80 p-3 space-y-1.5">
            <div className="flex items-center justify-between text-[11px]">
              <span className="font-bold text-emerald-400">
                {creators_service_1.DAILY_CHALLENGE.featuredResponse.authorName} ({creators_service_1.DAILY_CHALLENGE.featuredResponse.authorDistrict})
              </span>
              <span className="text-slate-400">🏆 Destaque</span>
            </div>
            <p className="text-xs text-slate-300 italic">
              "{creators_service_1.DAILY_CHALLENGE.featuredResponse.text}"
            </p>
          </div>)}

        <button type="button" onClick={onOpenCreateForChallenge} className="mt-3.5 w-full flex items-center justify-center gap-2 rounded-xl bg-amber-500 hover:bg-amber-400 text-slate-950 font-black text-xs uppercase tracking-wider py-2.5 transition-all cursor-pointer shadow-md">
      ...
      <div className="rounded-3xl border border-white/10 bg-slate-900/80 p-5 shadow-xl backdrop-blur-md space-y-3">
        ...
        <div className="space-y-2">
          {TRENDING_TOPICS.map(function (topic) { return (<button key={topic.tag} type="button" onClick={function () { return onSelectTopic && onSelectTopic(topic.tag.replace('#', '')); }} className="w-full flex items-center justify-between p-2 rounded-xl hover:bg-white/5 transition-colors text-left text-xs cursor-pointer group">
      ...
      <div className="rounded-3xl border border-emerald-500/30 bg-gradient-to-b from-emerald-950/30 via-slate-900 to-slate-950 p-5 shadow-xl backdrop-blur-md space-y-3">
        <div className="flex items-center gap-2 border-b border-white/10 pb-3">
          <lucide_react_1.Lightbulb className="h-4 w-4 text-emerald-400"/>
          <h3 className="font-display text-xs font-black uppercase tracking-wider text-white">
      ...
      <div className="rounded-3xl border border-white/10 bg-slate-900/80 p-5 shadow-xl backdrop-blur-md space-y-3">
        <div className="flex items-center justify-between border-b border-white/10 pb-3">
          <div className="flex items-center gap-2">
            <lucide_react_1.Crown className="h-4 w-4 text-amber-400"/>
            <h3 className="font-display text-xs font-black uppercase tracking-wider text-white">
              Criadores Destaque
            </h3>
          </div>
        ...
        <div className="space-y-3">
          {TOP_CREATORS.map(function (c, idx) { return (<link_1.default key={c.username} href={"/criadores/".concat(c.username)} className="flex items-center justify-between gap-2 p-1.5 rounded-2xl hover:bg-white/5 transition-colors group">
              <div className="flex items-center gap-2.5 min-w-0">
                <div className="relative">
                  <user_avatar_1.UserAvatar avatarUrl={c.avatar} size="sm"/>
