...
export default function GoogleAuthButton({ redirectTarget = '/jogar', onError }: GoogleAuthButtonProps) {
  ...
  const handleCredentialResponse = useCallback(
    async (response: any) => {
      try {
        ...
        setIsSigningIn(true)
        console.log('[AUTH GIS] Resposta de credencial recebida com sucesso. A autenticar no Firebase...')
  ...
  const handleManualFallback = async () => {
    ...
    try {
      ...
      setIsSigningIn(false)
      if (onError) onError(err?.message || 'Erro ao iniciar sessão com o Google.')
    }
  }
  ...
  return (
    <div className="w-full flex flex-col items-center justify-center my-2">
      ...
      {/* Fallback acessível se o GIS demorar a carregar */}
      {!gisLoaded && !isSigningIn && (
        <button
          type="button"
