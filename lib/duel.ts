...
// Lista de nomes e distritos portugueses para adversários simulados
const BOT_NAMES = [
  'Gonçalo_Porto',
  'Tiago_Lisboa',
  'Marta_Coimbra',
...
/**
 * Cria instantaneamente uma partida de Duelo 1v1 contra um Bot/Adversário Simulado
 * Executado quando a procura humana excede o tempo limite (6-8 segundos) para garantir zero espera.
 */
export async function createBotDuelMatch(
  user: { uid: string; displayName?: string | null; photoURL?: string | null },
  profile?: { level?: number; district?: string },
  myMatchAttemptId?: string,
): Promise<MatchmakingResult> {
  ...
  const now = Date.now()
  const myLevel = profile?.level || 1
  const botLevel = Math.max(1, Math.min(10, myLevel + (Math.random() > 0.5 ? 1 : 0)))
  const botName = BOT_NAMES[Math.floor(Math.random() * BOT_NAMES.length)]
  const botDistrict = BOT_DISTRICTS[Math.floor(Math.random() * BOT_DISTRICTS.length)]
  ...
  const duelDoc: DuelDocument = {
    ...
    code,
    status: 'matched',
    playerUids: [user.uid, botUid],
    matchAttemptA: myMatchAttemptId || crypto.randomUUID(),
    matchAttemptB: `attempt_${botUid}`,
    createdAt: now,
    startedAt: duelStartedAt,
    expiresAt,
    playerA,
  ...
  // Grava o documento do duelo no Firestore e atualiza o bilhete
  ...
  console.log('[MATCH BOT CREATED] duelId:', duelId, 'Adversário Simulado:', botName, 'Level:', botLevel)
  ...
  return {
    ...
    ticket: {
      ...
      district: playerA.district,
      status: 'matched',
      matchAttemptId: myMatchAttemptId || '',
      joinedAt: now,
      lastHeartbeat: now,
      expiresAt: now + 60_000,
      duelId,
