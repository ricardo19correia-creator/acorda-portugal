...
const app = initializeApp(firebaseConfig, 'audit-direct-' + Date.now())
...
async function runAudit() {
  ...
  try {
    ...
    if (humanCount > 0) {
      console.log('Humanos Encontrados:')
      humanXpList.forEach((h, idx) => {
    ...
    if (botCount > 0) {
      console.log('Top 10 Bots Atuais:')
      botXpList.slice(0, 10).forEach((b, idx) => {
    ...
    console.error('Erro na auditoria do Firestore:', error)
  }

  console.log('\n======================================================================')
}

runAudit().then(() => setTimeout(() => process.exit(0), 500))
