...
const batchFile = 'data/batches/batch_portugal_politico_constituicao_001.json'
const targetCategoryFile = 'lib/data/categories/portugal-politico.json'
...
batch.forEach((q) => {
  if (!existingIds.has(q.id)) {
    currentCategoryQuestions.push({
      ...q,
      category: 'Portugal Político',
      tema: 'Portugal Político',
      subtema: 'Constituição da República',
      subcategory: 'Constituição da República',
    })
...
prodLogs.push({
  batchId: 'BATCH_POL_CONST_001',
  timestamp: new Date().toISOString(),
  category: 'portugal-politico',
  theme: 'Portugal Político',
  subtema: 'Constituição da República',
  count: addedCount,
  qualityGate: 'PASSED',
  sourceVerified: true,
